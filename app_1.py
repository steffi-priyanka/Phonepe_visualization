...
# Plotly BAR PLOT_YEAR
    ...
    with col1:
        ...
        fig_tran_amount=px.area(tra_g,x="State", y= "Amount",title=f"{Year} TRANSACTION AMOUNT",
                               color_discrete_sequence=px.colors.sequential.Mint_r, height= 650,width=600)
        st.plotly_chart(fig_tran_amount)
    with col2:
        fig_tran_count = px.area(tra_g, x="State", y="Trans_count", title=f"{Year} TRANSACTION COUNT",
...
def agg_user_Q(df,Quarter):
    ...
    aguQ_fig_B_TC= px.pie(agu_Q_g, names="Brand", values="Count",
...
def agg_user_S(df,State):
    ...
    agu_S_fig_B_TC= px.area(agu_S, x="Brand", y="Count", pattern_shape="Percentage", width=1000,
...
def map_ins_D(df,State):
    ...
    col1,col2 = st.columns(2)
    with col1:
        mi_D_fig_DTA = px.area(mi_Dg, x="District", y="Amount", color="Amount",width=500, height=600, title=f"{State} DISTRICT & TRANSACTION AMOUNT",
                               color_discrete_sequence=px.colors.sequential.Mint_r)
        st.plotly_chart(mi_D_fig_DTA)
    with col2:
        mi_D_fig_DTC = px.area(mi_Dg, x="District", y="Trans_count", color="Trans_count",width=600, height=600, title=f"{State} DISTRICT & TRANSACTION COUNT",
...
def top_user_Y(df,Year):
    ...
    tpu_y_fig = px.scatter(tpu_y , x="State", y="User_count", width=1000, height=800, color="Quarter",
                             title= f"{Year} Registered_user_Count ",color_discrete_sequence=px.colors.sequential.Rainbow)
...
def top_chart_tran_amount(table_name):
    ...
    col1, col2 = st.columns(2)
    with col1:
        fig_amount = px.pie(df1, names="State", values="Amount", title="TOP 10 Transaction Amount",
...
# QUERY 2_ASCENDING TRANSACTION_AMOUNT
    ...
    with col2:
        fig_amount2 = px.bar(df2, x="State", y="Amount", title="LEAST 10_Transaction Amount", hover_name="State",
...
# QUERY 3_ AVERAGE TRANSACTION_AMOUNT
    ...
    fig_amount3 = px.scatter(df3, x="State", y="Amount", title="AVERAGE_Transaction Amount", hover_name="State",
...
def top_chart_tran_count(table_name):
    ...
    col1, col2 = st.columns(2)
    with col1:
        fig_count = px.area(df1, x="State", y="Trans_count", title="TOP 10 Transaction Count", hover_name="State", color ="Trans_count",
                             color_discrete_sequence=px.colors.sequential.deep_r, height=650, width=500)
...
# QUERY 2_ASCENDING TRANSACTION_COUNT
    ...
    with col2:
        fig_count2 = px.bar(df2, x="State", y="Trans_count", title="LEAST 10_Transaction Count", hover_name="State",
                             color_discrete_sequence=px.colors.sequential.Blues_r, height=650, width=500)
...
def top_chart_regis_users(table_name,State):
    ...
    col1, col2 = st.columns(2)
    with col1:
        fig_amount = px.area(df_1, x="District", y="Registered_users", title="TOP 10 REGISTERED USERS", color="Registered_users",
                            color_discrete_sequence=px.colors.sequential.Blues_r, height=650, width=500)
        st.plotly_chart(fig_amount)
...
# LAST 10 REGISTERED USER
    ...
    with col2:
        ...
        fig_amount_2= px.pie(df_2, names="District", values="Registered_users", title="LAST 10 REGISTERED USERS", hover_name= "District",
                            color_discrete_sequence=px.colors.sequential.Rainbow, height= 650,width= 500)
 ...
 # AVERAGE REGISTERED USERS
    ...
    fig_amount_3= px.line(df_3, x="District", y="Registered_users", title="AVERAGE REGISTERED USERS", markers= True,
...
def top_chart_app_open(table_name,State):
    ...
    col1, col2 = st.columns(2)
    with col1:
        fig_amount = px.pie(df_1, names="District", values="App_open", title="TOP 10 App_open",
                            hover_name="District",
                            color_discrete_sequence=px.colors.sequential.Blues_r, height=650, width=500)
        st.plotly_chart(fig_amount)
    ...
    with col2:
        fig_amount_2 = px.area(df_2, x="District", y="App_open", title="LAST 10 App_open", color="App_open",
                              hover_name="District",
    ...
    fig_amount_3 = px.line(df_3, y="District", x="App_open", title="AVERAGE App_open", markers=True,
                          hover_name="District", orientation="h",
                          color_discrete_sequence=px.colors.sequential.Emrld_r, height=800, width=1000)
    st.plotly_chart(fig_amount_3)
...
def top_chart_user_count(table_name):
    ...
    with col1:
        fig_count = px.bar(df1, x="State", y="User_count", title="TOP 10 User_count", hover_name="State",
                           color_discrete_sequence=px.colors.sequential.Agsunset_r, height=650, width=500)
        st.plotly_chart(fig_count)
    ...
    with col2:
        fig_count = px.pie(df2, names="State", values="User_count", title="LEAST 10 User_count",
    ...
    fig_count = px.line(df3, x="State", y="User_count", title="AVERAGE User_count",markers=True,
                        hover_name="User_count", color_discrete_sequence=px.colors.sequential.Rainbow, height=800, width=800)
...
st.title(":Green[PHONEPE DATA ANALYSIS]")
select = option_menu(None, ["Home", "Explore Data", "Top charts"], orientation="horizontal")
...
# #______________________________________EXPLORE DATA ______________________
elif select=="Explore Data":
    tab1, tab2, tab3 = st.tabs(["Aggregated Analysis", "Map Analysis", "Top Analysis"])

    with tab1:
        method = st.selectbox("Select the Method",["Agg Insurance", "Agg Transaction","Agg User"])
...
#
    with tab2:
        map_method = st.selectbox("Select the Method", ["Map Insurance", "Map Transaction", "Map User"])
        ...
        # STATE
            with col1:
                State = st.selectbox("Select the STATE", map_ins_Y["State"].unique())
        ...
        # DISTRICT
            ...
            col1, col2 = st.columns(2)
            with col1:
                Year = st.slider("SELECT YEAR to view MAP INSURANCE", min_value=2018, max_value=2023)
...
#
    with tab3:
        top_method = st.selectbox("Select the Method", ["Top Insurance", "Top Transaction", "Top User"])
#
        ...
        # STATE
            col1, col2 = st.columns(2)
            with col1:
                State = st.selectbox("Select State to view District", top_ins_Y["State"].unique())
...
#     #--------------------------------------------------------------------------------------
            ...
            col1, col2 = st.columns(2)
            with col1:
                Year = st.slider("SELECT YEAR to view TOP TRANSACTION", min_value=2018, max_value=2023)
    ...
    #-------------------------------------------------------------------------------------
            ...
            col1, col2 = st.columns(2)
            with col1:
                Year = st.slider("SELECT YEAR to view TOP USER", min_value=2018, max_value=2023)
...
#-----------------------------------------------------------------------------------------------------
    ...
    question = st.selectbox("Choose to Visualize Report", ["1. Transaction Amount and Count of Aggregated Insurance",
